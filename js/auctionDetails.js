...
if (getCookie('isConnect') == 'false' || getCookie('islogin') == 'false') {
	$('.bids-list').hide()
	$('.bid-right-btn').css('color', '#9567FF')
	$('.bid-right-btn').html(chEnText.auction[lang].bidText)
} else {
	initialization()
}
//普通弹窗 
		...
		.then(function (res) {
			...
			auctionContractInstance.methods._auctions(tokenTypeId).call()
				.then(function (res) {
					...
					$('.bid-right-status-current span:nth-child(2)').text('BUSD 22,000');
