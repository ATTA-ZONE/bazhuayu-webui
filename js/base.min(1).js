! function (W) {
	...
	W.conf = {
		'dev': top.location.origin,
		'test': 'https://www.bazhuayu.io',
		'prod': 'https://www.bazhuayu.io',
		1: 'https://www.bazhuayu.io',
		56: 'https://www.bazhuayu.io',
		97: 'https://www.bazhuayu.io'
	};
	...
	location.query = function (a, d) {
		var c = W.location.search.substr(1),
			b = c.match(new RegExp("(^|&)" + a + "=([^&]*)(&|$)"));
		if (d) {
		...
		if (js.readyState) {
			js.onreadystatechange = function () {
				if (js.readyState == 'loaded' || js.readyState == 'complete') {
					js.onreadystatechange = null, cb && cb()
				}
			}
		} else {
			js.onload = cb
	...
	W.cookie = function (k, v, o, path, domain, secure) {
	...
	W.CHAIN = {
		...
		WALLET: {
			...
			handleChainChanged: function (chainId) {
				...
				var code = chainId,
					address = CHAIN.WALLET.walletAddress();
			...
			WalletConnect: {
				...
				connect: function (c) {
					...
					provider.enable().then(function (accounts) {
						var wallet = CHAIN.WALLET.__wallet__,
							addr = accounts[0].toLocaleLowerCase();
		...
		invoke: function () {
			...
			var X = [A, I].join('.'),
				...
				C = function () {
					...
					c.apply(c, args)[CN || "call"]((CD || {
						from: CHAIN.WALLET.walletAddress()
					}), CB);
				};
